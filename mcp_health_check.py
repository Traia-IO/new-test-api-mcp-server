#!/usr/bin/env python
"""
New Test API MCP Server Health Check Script

This script properly connects to the New Test API MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if New Test API MCP server is healthy"""
    print(f"🔍 Checking New Test API MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected New Test API tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check New Test API MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="New Test API MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 New Test API MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ New Test API MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ New Test API MCP Server health check failed!")
        return 1
