#!/usr/bin/env python3
"""
New Test API MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('new-test-api_mcp')
...
logger.info("="*80)
logger.info(f"New Test API MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://api.apis.guru/v2/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("New Test API MCP Server", host="0.0.0.0")
...
# - NEW_TEST_API_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="new-test-api-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "new-test-api-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting New Test API MCP Server")
    logger.info("="*80)
