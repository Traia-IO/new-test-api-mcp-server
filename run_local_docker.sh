...
# Script to build and run the New Test API MCP Server locally in Docker
...
IMAGE_NAME="new-test-api-mcp-server"
CONTAINER_NAME="new-test-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running New Test API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# New Test API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ New Test API MCP Server is running!${NC}"
echo
